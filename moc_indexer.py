...
class MoCIndexer:
    ...
    def moc_state_from_sc(self, block_identifier: BlockIdentifier = 'latest'):
        ...
        d_moc_state["b0Leverage"] = str(
            self.contract_MoCState.leverage(
                bucket_c0,
                formatted=False,
                block_identifier=block_identifier))
        d_moc_state["b0TargetCoverage"] = str(
            self.contract_MoCState.cobj(
                formatted=False,
                block_identifier=block_identifier))
        ...
        d_moc_state["b0BTCAmount"] = str(
            self.contract_MoCState.bucket_nbtc(
                bucket_c0,
                formatted=False,
                block_identifier=block_identifier))
        d_moc_state["b0DocAmount"] = str(
            self.contract_MoCState.bucket_ndoc(
                bucket_c0,
                formatted=False,
                block_identifier=block_identifier))
        d_moc_state["b0BproAmount"] = str(
            self.contract_MoCState.bucket_nbpro(
                bucket_c0,
                formatted=False,
                block_identifier=block_identifier))
        ...
        d_moc_state["x2Coverage"] = str(
            self.contract_MoCState.coverage(
                bucket_x2,
                formatted=False,
                block_identifier=block_identifier))
        ...
        d_moc_state["bproPriceInUsd"] = str(
            self.contract_MoCState.bpro_price(
                formatted=False,
                block_identifier=block_identifier))
        ...
        try:
            d_moc_state["spotInrate"] = str(
                self.contract_MoCInrate.spot_inrate(
                    formatted=False,
                    block_identifier=block_identifier))
        except HTTPError:
    ...
    def search_moc_transaction(self, block):
        ...
        d_block = chain.get_block(block)

        l_transactions, d_index_transactions = filter_transactions(
    ...
    def logs_process_moc_exchange(self,
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'RiskProMint' in tx_event:
                d_event = MoCExchangeRiskProMint(tx_event['RiskProMint'],
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'RiskProRedeem' in tx_event:
                d_event = MoCExchangeRiskProRedeem(tx_event['RiskProRedeem'],
                                                   tx_receipt=tx_receipt)
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'RiskProxMint' in tx_event:
                d_event = MoCExchangeRiskProxMint(tx_event['RiskProxMint'],
                                                  tx_receipt=tx_receipt)
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'RiskProxRedeem' in tx_event:
                d_event = MoCExchangeRiskProxRedeem(tx_event['RiskProxRedeem'],
                                                    tx_receipt=tx_receipt)
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'StableTokenMint' in tx_event:
                d_event = MoCExchangeStableTokenMint(tx_event['StableTokenMint'],
                                                     tx_receipt=tx_receipt)
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'StableTokenRedeem' in tx_event:
                d_event = MoCExchangeStableTokenRedeem(tx_event['StableTokenRedeem'],
                                                       tx_receipt=tx_receipt)
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'FreeStableTokenRedeem' in tx_event:
                d_event = MoCExchangeFreeStableTokenRedeem(tx_event['FreeStableTokenRedeem'],
                                                           tx_receipt=tx_receipt)
    ...
    def moc_settlement_redeem_stable_token_notification(self,
                                                        tx_receipt,
                                                        tx_event,
                                                        tx_log,
                                                        m_client):
    ...
    def logs_process_moc_settlement(self,
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'SettlementStarted' in tx_event:
                d_event = MoCSettlementSettlementStarted(tx_event['SettlementStarted'],
                                                         tx_receipt=tx_receipt)
                self.moc_settlement_started(
                    tx_receipt,
                    d_event.event,
                    m_client)
                self.update_settlement_state(
                    tx_event,
                    m_client,
                    block_ts)
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'RedeemRequestAlter' in tx_event:
                d_event = MoCSettlementRedeemRequestAlter(tx_event['RedeemRequestAlter'],
                                                          tx_receipt=tx_receipt)
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'RedeemRequestProcessed' in tx_event:
                d_event = MoCSettlementRedeemRequestProcessed(tx_event['RedeemRequestProcessed'],
                                                              tx_receipt=tx_receipt)
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'SettlementRedeemStableToken' in tx_event:
                d_event = MoCSettlementSettlementRedeemStableToken(tx_event['SettlementRedeemStableToken'],
                                                                   tx_receipt=tx_receipt)
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'SettlementDeleveraging' in tx_event:
                d_event = MoCSettlementSettlementDeleveraging(tx_event['SettlementDeleveraging'],
                                                              tx_receipt=tx_receipt)
                self.moc_settlement_deleveraging(tx_receipt,
                                                 ...
                                                 d_moc_transactions,
                                                 block_ts)
                self.set_settlement_state(tx_event,
                                          m_client,
                                          block_ts)
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'SettlementCompleted' in tx_event:
                d_event = MoCSettlementSettlementCompleted(tx_event['SettlementCompleted'],
                                                           tx_receipt=tx_receipt)
    ...
    def logs_process_moc_inrate(self, tx_receipt, m_client, block_ts):
        ...
        for tx_event in tx_receipt.events:
            ...
            if str(raw_logs[tx_index]['address']).lower() != str(inrate_address).lower():
                continue

            if 'InrateDailyPay' in tx_event:
                d_event = MoCInrateDailyPay(tx_event['InrateDailyPay'],
                                            tx_receipt=tx_receipt)
                self.moc_inrate_daily_pay(
                    tx_receipt,
                    d_event.event,
                    m_client,
                    block_ts)
        ...
        for tx_event in tx_receipt.events:
            ...
            if 'RiskProHoldersInterestPay' in tx_event:
                d_event = MoCInrateRiskProHoldersInterestPay(
                    tx_event['RiskProHoldersInterestPay'],
                    tx_receipt=tx_receipt)
                self.moc_inrate_risk_pro_holders_interest_pay(
                    tx_receipt,
                    d_event.event,
                    m_client,
                    block_ts)
    ...
    def logs_process_moc(self,
        ...
        for tx_event in tx_receipt.events:
            ...
            if str(raw_logs[tx_index]['address']).lower() != str(moc_address).lower():
                continue
    ...
    def logs_process_moc_state(self, tx_receipt, m_client):
        ...
        for tx_event in tx_receipt.events:
            ...
            if str(raw_logs[tx_index]['address']).lower() != str(self.contract_MoCState.address()).lower():
                continue

            if 'StateTransition' in tx_event:
                d_event = MoCStateStateTransition(tx_event['StateTransition'],
                                                  tx_receipt=tx_receipt)
                self.moc_state_transition(
                    tx_receipt,
                    d_event.event,
                    m_client)
    ...
    def logs_process_transfer(self,
        ...
        for tx_event in tx_receipt.events:
            ...
            if str(raw_logs[tx_index]['address']).lower() != str(token_riskpro).lower():
                continue

            if 'Transfer' in tx_event:
                d_event = ERC20Transfer(tx_event['Transfer'],
                                        tx_receipt=tx_receipt)
        ...
        for tx_event in tx_receipt.events:
            ...
            if str(raw_logs[tx_index]['address']).lower() != str(token_stable).lower():
                continue
        ...
        if self.app_mode == 'RRC20':
            ...
            for tx_event in tx_receipt.events:
                ...
                if str(raw_logs[tx_index]['address']).lower() != str(token_reserve).lower():
                    continue
    ...
    def logs_process_reserve_approval(self, tx_receipt, m_client):
        ...
        for tx_event in tx_receipt.events:
            ...
            if str(raw_logs[tx_index]['address']).lower() != str(self.contract_ReserveToken.address()).lower():
                continue

            if 'Approval' in tx_event:
                d_event = ERC20Approval(tx_event['Approval'],
                                        tx_receipt=tx_receipt)
                self.update_user_state_approval(d_event.event, m_client)
